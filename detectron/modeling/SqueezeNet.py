...
class SqueezeNet:
  ...
  def fire_module(self,x,inp,fire_id,sq,exp_):
    ...
    return brew.concat(self.model,[left,right],s_id + 'concat',axis=self.data_format),exp_*2
  def create_model(self,x):
    x = brew.conv(self.model, x, 'conv1',3, 64, 3,stride=2)
    x = brew.relu(self.model, x,'relu_conv1')
    x = brew.max_pool(self.model, x, 'pool1', kernel=3, stride=2)
    x,inp = self.fire_module(x, fire_id=2, sq=16, exp_=64,inp=64)
    x,inp = self.fire_module(x, fire_id=3, sq=16, exp_=64,inp=inp)
    x = brew.max_pool(self.model, x, 'pool3', kernel=3, stride=2)
    x,inp = self.fire_module(x, fire_id=4, sq=32, exp_=128,inp=inp)
    x,inp = self.fire_module(x, fire_id=5, sq=32, exp_=128,inp=inp)
    x = brew.max_pool(self.model, x, 'pool5', kernel=3, stride=2)
